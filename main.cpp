...
class Rook;
class Player;
// Global pointer to track last double-step pawn move for en passant
...
public:
    ...
    void initializeSprite( std::map<std::string, sf::Texture>& textureMap) {
        ...
        sprite.setPosition(position.col * 80.0f, position.row * 80.0f); // 80px squares
    }
...
class Board {
public:
    Piece*** board; // Pointer to 2D array of Piece*
    Board() {
        board = new Piece * *[8];
        for (int i = 0; i < 8; ++i) {
            board[i] = new Piece * [8];
    ...
    // SFML: Call every frame to draw all pieces
...
public:
                ...
                if (piece && piece->get_color() == color) {
                    ...
                    for (int r = 0; r < 8; r++) {
                        for (int c = 0; c < 8; c++) {
                            ...
                            // Backup board and piece positions
    ...
    bool isStalemate() {
        if (isCheck()) return false; // Stalemate requires NOT being in check
                ...
                if (piece && piece->get_color() == color) {
                    ...
                    for (int r = 0; r < 8; r++) {
                        for (int c = 0; c < 8; c++) {
                            ...
                            // If even one legal move exists that doesn't cause check, not stalemate
                            if (!stillInCheck) return false;
                        }
                    }
                }
...
start_game:
    ...
    board.board[0][4] = new King(1, Position(0, 4));    // Assuming King class will be added next
    ...
    board.board[7][4] = new King(0, Position(7, 4));    // Assuming King class will be added next
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (!pieceSelected) {
                    ...
                    if (p) {
                        ...
                        if (p->get_color() == currentTurnColor) {
                            selected = clicked;
                            pieceSelected = true;
                        }
                    }
